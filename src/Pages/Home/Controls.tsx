...
const Controls: React.FC<ControlsProps> = ({ darkMode, toggleDarkMode, titleVariant, toggleTitleVariant }) => {
    ...
    const switchVariant = {
        on: { x: "+175%", backgroundColor: "#BC1ECA" },
        off: { x: "0", backgroundColor: "#7D2BDF" },
    }
    ...
    return (
        <motion.section
            ...
            className='w-screen min-h-togglePanel bg-dark rounded-3xl rotate-12 relative -top-80 -right-36rem overflow-hidden p-16 dark:bg-gray'>
            ...
            <button
                onClick={fetchJoke}
                className='rounded-lg bg-purple text-white font-bold px-8 py-4 text-xl mt-16 hover:bg-pink transition-colors duration-500'>Treat</button>
